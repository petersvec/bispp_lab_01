...
class User(object):
    ...
    def setPerson(self, username, token):
        ...
        self.token = token
        self.fcoins = bank.balance(username)

def logged_in():
    g.user = User()
    g.user.checkCookie(request.cookies.get("PyFBankLogin"))
    if g.user.person:
        return True
    else:
        return False
...
def login():
    ...
    if cookie:
        ...
        response.set_cookie('PyFBankLogin', cookie)
        return response
...
def logout():
    ...
    response.set_cookie('PyFBankLogin', '')
    return response
